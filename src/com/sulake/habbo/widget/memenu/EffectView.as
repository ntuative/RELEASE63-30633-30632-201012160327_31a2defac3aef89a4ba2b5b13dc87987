package com.sulake.habbo.widget.memenu
{
   ...
   public class EffectView
   {
      ...
      private var var_160:IWidgetAvatarEffect;
      
      private var var_855:IWindow;
      ...
      {
         super();
         this.var_174 = new Timer(1000);
         this.var_174.addEventListener(TimerEvent.TIMER,this.onUpdate);
      }
      ...
      public function dispose() : void
      {
         ...
         {
            this.var_174.stop();
            this.var_174.removeEventListener(TimerEvent.TIMER,this.onUpdate);
            this.var_174 = null;
         }
      ...
      private function onUpdate(param1:Event = null) : void
      {
         ...
         {
            if(this.var_160.isActive)
            {
               _loc2_ = this.var_160.secondsLeft / Number(this.var_160.duration);
               this.var_855.width = _loc2_ * this.var_463;
            }
            else
            {
               this.var_855.width = 0;
               this.var_174.stop();
            }
            this.setTimeLeft();
         }
      ...
      private function setTimeLeft() : void
      {
         ...
         {
            this.var_857.caption = "${widgets.memenu.effects.active.timeleft}";
            _loc1_ = this.var_857.text;
            if(this.var_160.isActive)
            {
               _loc2_ = this.var_160.secondsLeft;
               ...
               this.var_857.text = _loc1_;
            }
            else
            {
               this.var_857.caption = "${widgets.memenu.effects.activate}";
            }
         }
      }
      ...
      private function createWindow(param1:String) : void
      {
         ...
         var _loc7_:* = null;
         var _loc3_:String = "";
         if(this.var_160.isInUse)
         {
            _loc3_ = "memenu_effect_selected";
         }
         else if(this.var_160.isActive)
         {
         ...
         else
         {
            ...
            this.var_856 = this._window.findChildByName("effect_hilite");
            this.var_856.visible = false;
         }
         this.var_857 = this._window.findChildByName("time_left") as ITextWindow;
         this.setTimeLeft();
         this.var_855 = this._window.findChildByName("loader_bar");
         if(this.var_855 != null)
         {
            this.var_463 = this.var_855.width;
            this.var_174.start();
            this.onUpdate();
         }
         if(this.var_160.icon)
         {
            this.setElementBitmap("effect_icon",this.var_160.icon);
         }
      }
