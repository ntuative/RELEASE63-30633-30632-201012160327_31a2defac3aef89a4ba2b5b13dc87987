package com.sulake.habbo.communication.demo
{
   ...
   public class SSOTicket extends EventDispatcher
   {
      ...
      public static const const_354:String = "success";
      
      public static const const_305:String = "failure";
      ...
      private var var_1386:IHabboWebLogin;
      ...
      public function SSOTicket(param1:IAssetLibrary, param2:IHabboWebLogin, param3:String, param4:Boolean = true)
      {
         ...
         this.var_1385 = "http://" + param3 + "/client";
         if(!param4)
         {
            this.var_1386.init();
            this.var_1386.addEventListener(HabboWeb.const_247,this.requestClientURL);
         }
         else
         {
      ...
      {
         var _loc2_:String = this.var_1385;
         if(this._assets.hasAsset(_loc2_))
         {
            Logger.log("[CoreLocalizationManager] reload localization for url: " + this.var_1385);
         }
         var _loc3_:URLRequest = new URLRequest(this.var_1385);
         if(this._assets.hasAsset(_loc2_))
         {
      ...
      private function clientURLComplete(param1:Event = null) : void
      {
         ...
         {
            this.var_1386.requestReAuthenticate();
         }
         else
         {
            try
            {
               ...
               this.var_2498 = /\"flash.client.url\" : \"(.*?)\"/.exec(data)[1];
               facebookData = /\"facebook.user\" : \"(.*?)\"/.exec(data);
               if(facebookData)
               {
                  this.var_2497 = Boolean(facebookData[1]);
               }
               if(this.var_1771.length > 0)
               {
                  dispatchEvent(new Event(const_354));
               }
               else
               {
