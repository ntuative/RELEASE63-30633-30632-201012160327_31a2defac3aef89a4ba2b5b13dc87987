package com.sulake.habbo.roomevents.userdefinedroomevents.help
{
   ...
   public class UserDefinedRoomEventsHelp
   {
      ...
      private var var_992:IContainerButtonWindow;
      
      private var var_991:IContainerButtonWindow;
      ...
      private function prepareWindow() : void
      {
         ...
         this._window = IFrameWindow(this.var_89.getXmlWindow("ude_help"));
         this.var_1533 = IWindowContainer(this.find(this._window,"help_container"));
         this.var_991 = this.var_1533.findChildByName("button_previous") as IContainerButtonWindow;
         if(this.var_991)
         {
            this.var_991.procedure = this.onButtonClick;
         }
         this.var_992 = this.var_1533.findChildByName("button_next") as IContainerButtonWindow;
         if(this.var_992)
         {
            this.var_992.procedure = this.onButtonClick;
         }
      ...
      {
         if(this.var_226 >= this._localizations.length - 1)
         {
            this.var_992.disable();
         }
         else
         {
            this.var_992.enable();
      ...
      {
         ++this.var_226;
         this.var_226 = Math.min(this.var_226,this._localizations.length - 1);
         this.openPage();
      }
      ...
      {
         --this.var_226;
         this.var_226 = Math.max(this.var_226,0);
         this.openPage();
      }
      ...
      private function initLocalizations() : void
      {
         ...
         this._localizations = [];
         var _loc1_:int = 1;
         while(this.var_89.localization.getLocalization("wiredfurni.help." + _loc1_ + ".title"))
         {
            _loc2_ = this.var_89.localization.getLocalization("wiredfurni.help." + _loc1_ + ".title");
            _loc3_ = this.var_89.localization.getLocalization("wiredfurni.help." + _loc1_ + ".img");
            _loc4_ = this.var_89.localization.getLocalization("wiredfurni.help." + _loc1_ + ".text");
      ...
      private function localize() : void
      {
         ...
         if(_loc1_)
         {
            _loc1_.text = ILocalization(this._localizations[this.var_226][0]).value;
         }
         if(_loc2_)
         {
            _loc2_.text = ILocalization(this._localizations[this.var_226][2]).value;
         }
         if(_loc3_)
         {
            _loc4_ = ILocalization(this._localizations[this.var_226][1]).value;
      ...
      {
         if(!param1 || !this.var_89 || !this.var_89.assets)
         {
            return;
         }
         var _loc2_:BitmapDataAsset = this.var_89.assets.getAssetByName(param1) as BitmapDataAsset;
         if(_loc2_ == null)
         {
      ...
      private function retrievePreviewAsset(param1:String) : void
      {
         ...
         var _loc2_:String = this.var_89.configuration.getKey("image.library.catalogue.url");
         var _loc3_:* = _loc2_ + param1 + ".gif";
         Logger.log("[ProductViewCatalogWidget] Retrieve Product Preview Asset: " + _loc3_);
         var _loc4_:URLRequest = new URLRequest(_loc3_);
         var _loc5_:AssetLoaderStruct = this.var_89.assets.loadAssetFromFile(param1,_loc4_,"image/gif");
         if(!_loc5_)
         {
            return;
         }
