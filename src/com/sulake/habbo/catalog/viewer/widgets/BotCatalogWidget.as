package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class BotCatalogWidget extends CatalogWidget implements ICatalogWidget, IItemGrid, IAvatarImageListener
   {
      ...
      private var var_693:Offer;
      
      protected var var_1175:IItemGridWindow;
      
      protected var var_1623:XML;
      
      private var var_1685:Map;
      
      private var var_328:IGridItem;
      
      public function BotCatalogWidget(param1:IWindowContainer)
      {
         super(param1);
         this.var_1175 = _window.findChildByName("botGrid") as IItemGridWindow;
         this.var_1685 = new Map();
      }
      ...
      {
         (page.viewer.catalog as HabboCatalog).showPurchaseConfirmation(this.var_693,page.pageId);
      }
      
      private function onBotDefinitions(param1:RentableBotDefinitionWidgetEvent) : void
      {
         this.var_1329 = param1.botFigureData;
      ...
      {
         var _loc6_:* = null;
         var _loc2_:IWindowContainer = page.viewer.catalog.windowManager.buildFromXML(this.var_1623) as IWindowContainer;
         this.var_1175.addGridItem(_loc2_);
         param1.productContainer.view = _loc2_;
         var _loc3_:String = this.var_1329.getValue(param1.productContainer.firstProduct.extraParam.toLowerCase());
      ...
      {
         var _loc1_:String = this.var_1329.getValue(this.var_693.productContainer.firstProduct.extraParam.toLowerCase());
         var _loc2_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createAvatarImage(_loc1_,AvatarScaleType.const_46);
         if(_loc2_)
         {
            _loc2_.setDirection(AvatarSetType.const_32,4);
            this.setPreviewImage(_loc2_.getImage(AvatarSetType.const_32,true));
            _loc2_.dispose();
         }
      }
      ...
      private function setProductData() : void
      {
         ...
         if(_loc2_ != null)
         {
            ...
            _loc1_.registerParameter(_loc5_,"credits",String(this.var_693.priceInCredits));
            _loc1_.registerParameter(_loc5_,"pixels",String(this.var_693.priceInActivityPoints));
            _loc2_.text = _loc1_.getKey(_loc5_);
         }
         var _loc3_:IProductData = page.viewer.catalog.getProductData(this.var_693.localizationId);
      ...
      public function avatarImageReady(param1:String) : void
      {
         ...
         var _loc7_:* = null;
         var _loc8_:* = null;
         var _loc2_:int = this.var_1685.getValue(param1);
         if(true)
         {
            return;
         }
         for each(_loc3_ in page.offers)
         {
            ...
            {
               _loc4_ = this.var_1329.getValue(_loc3_.productContainer.firstProduct.extraParam.toLowerCase());
               ...
               if(_loc7_)
               {
                  _loc7_.setDirection(AvatarSetType.const_32,4);
                  _loc8_ = _loc7_.getImage(AvatarSetType.const_32,true);
                  _loc6_.bitmap.copyPixels(_loc8_,_loc8_.rect,new Point((_loc6_.width - _loc8_.width) / 2,_loc6_.height - _loc8_.height));
                  _loc6_.invalidate();
                  _loc7_.dispose();
                  if(this.var_693.offerId == _loc2_)
                  {
                     this.updatePreviewImage();
                  }
               }
