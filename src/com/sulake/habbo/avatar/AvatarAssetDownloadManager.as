package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_897:int = 3;
      
      private var var_393:AssetLoaderStruct;
      
      private var var_898:Timer;
      ...
      private const const_1850:int = 100;
      
      private const const_1336:int = 2;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         this.var_896 = new Dictionary();
         this.var_2479 = param3;
         this.var_1378 = param2;
         this.var_470 = new Dictionary();
         this.var_1111 = [];
         this.var_727 = [];
         this.var_726 = [];
         ...
         if(_loc6_ == null)
         {
            this.var_393 = this._assets.loadAssetFromFile("figuremap",_loc5_,"text/xml");
         ...
         this.var_898 = new Timer(this.const_1850,1);
         this.var_898.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_393)
         {
            this.var_393.addEventListener(AssetLoaderEvent.const_30,this.onConfigurationComplete);
            this.var_393.addEventListener(AssetLoaderEvent.const_42,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_393)
         {
            this.var_393.removeEventListener(AssetLoaderEvent.const_30,this.onConfigurationComplete);
            this.var_393.removeEventListener(AssetLoaderEvent.const_42,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_897);
            if(this.var_1378.indexOf("?") > 0)
            {
               _loc2_ = this.var_1378 + "&retry=" + this.var_897;
            }
            else
            {
               _loc2_ = this.var_1378 + "?retry=" + this.var_897;
            ...
            this.var_393 = this._assets.loadAssetFromFile("figuremap",_loc3_,"text/xml");
            this.addMapLoaderEventListeners();
         }
      }
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2479,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1110[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(_loc3_);
               this.var_1110[_loc5_] = _loc6_;
            }
         }
      }
      ...
      public function loadFigureSetData(param1:AvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1757)
         {
            this.var_1111.push([param1,param2]);
            return;
         }
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = this.var_470[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               this.var_470[_loc3_] = _loc6_;
            }
            this.var_896[_loc3_] = _loc4_;
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         var _loc10_:* = null;
         var _loc2_:* = [];
         for(_loc3_ in this.var_896)
         {
            _loc7_ = true;
            _loc4_ = this.var_896[_loc3_];
            ...
            if(_loc7_)
            {
               ...
               delete this.var_470[_loc3_];
            }
         }
         for each(_loc3_ in _loc2_)
         {
            delete this.var_896[_loc3_];
         }
         while(_loc6_ < this.var_726.length)
         {
            _loc5_ = this.var_726[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_726.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_898.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_727.length > 0 && this.var_726.length < this.const_1336)
         {
            _loc1_ = this.var_727[0];
            _loc1_.startDownloading();
            this.var_726.push(this.var_727.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_727.indexOf(param1) == -1 && this.var_726.indexOf(param1) == -1)
         {
