package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_728:IFigurePartSet;
      ...
      private var var_597:Rectangle;
      
      private var var_1767:int = 0;
      
      private var var_395:IAvatarRenderManager;
      
      private var var_645:Boolean;
      
      private var var_1766:BitmapData;
      
      private const const_1843:int = 13421772;
      
      private const const_1842:int = 16777215;
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         if(param3 == null)
         {
            this.var_624 = new BitmapData(1,1,true,16777215);
         }
         if(param3 != null)
         {
            _loc7_ = param3.parts;
            ...
            {
               this.var_1767 = Math.max(this.var_1767,_loc6_.colorLayerIndex);
            }
         }
         this.var_395 = this.var_62.controller.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_624 != null && !this.var_1765)
            {
               _loc3_ = this.var_624;
            }
            else
         ...
         if(_loc2_)
         {
            if(this.var_728)
            {
               switch(this.var_728.clubLevel)
               {
                  case HabboClubLevelEnum.const_49:
                     _loc2_.visible = false;
                     break;
                  case HabboClubLevelEnum.const_58:
                     _loc2_.style = HabboIconType.const_561;
                     break;
                  case HabboClubLevelEnum.const_50:
                     _loc2_.style = HabboIconType.const_607;
               }
            }
            else
            {
         ...
         if(this.isSelected)
         {
            this._window.color = this.const_1843;
         }
         else
         {
            this._window.color = this.const_1842;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:AvatarFigureContainer = this.var_395.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_395.isFigureReady(_loc1_))
         {
            this.var_395.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_311[_loc6_] + "_" + FigureData.const_130;
               _loc4_ = this.var_395.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_597 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1766)
               {
                  ...
                  this.var_1766 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_597.width,this.var_597.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_597.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_597.y;
               if(this.var_1765 && _loc2_.colorLayerIndex > 0)
               {
