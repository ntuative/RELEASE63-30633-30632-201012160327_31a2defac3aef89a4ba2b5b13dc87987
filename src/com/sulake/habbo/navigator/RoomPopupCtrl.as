package com.sulake.habbo.navigator
{
   ...
   public class RoomPopupCtrl extends PopupCtrl
   {
      ...
      private var var_104:GuestRoomData;
      
      private var var_2019:RoomThumbnailRenderer;
      ...
      public function RoomPopupCtrl(param1:HabboNavigator, param2:int, param3:int)
      {
         ...
         this.var_2019 = new RoomThumbnailRenderer(param1);
         this.var_960 = new TagRenderer(param1,hideInstantly);
      }
      ...
      override public function refreshContent(param1:IWindowContainer) : void
      {
         ...
         {
            this.var_153 = IWindowContainer(param1.findChildByName("details_container"));
         }
         this.var_153.visible = true;
         Util.hideChildren(this.var_153);
         this.refreshRoomName(this.var_153,this.var_104);
         if(this.var_104.ownerName != "" && this.var_104.ownerName != "-")
         {
         }
         this.refreshOwnerName();
         this.refreshTextWithCaption("roomctg",this.var_153,!!this.var_104.event ? this.getEventCtg(this.var_104.categoryId) : this.getRoomCtg(this.var_104.categoryId));
         this.refreshRoomDesc(this.var_153,this.var_104);
         this.refreshExtraCont();
         this.refreshInfo(this.var_153,"trading_allowed",this.var_104.allowTrading);
         this.refreshInfo(this.var_153,"doormode_doorbell",this.var_104.doorMode == 1);
         this.refreshInfo(this.var_153,"doormode_password",this.var_104.doorMode == 2);
         Util.moveChildrenToColumn(this.var_153,["roomname","roomctg_cont","roomowner_cont","roomdesc","extra_cont","doormode_doorbell","doormode_password","trading_allowed"],0,0);
         this.var_153.height = Util.getLowestPoint(this.var_153);
      }
      
      private function refreshOwnerName() : void
      {
         this.refreshTextWithCaption("roomowner",this.var_153,this.var_104.ownerName);
         var _loc1_:ITextWindow = ITextWindow(this.var_153.findChildByName("roomowner"));
         if(this.var_104.ownerName != "" && this.var_104.ownerName != "-")
         {
      ...
      {
         var _loc1_:IWindowContainer = IWindowContainer(this.var_153.findChildByName("extra_cont"));
         Util.hideChildren(_loc1_);
         this.var_960.refreshTags(_loc1_,this.var_104.tags);
         this.refreshTextWithCaption("rating",_loc1_,"" + this.var_104.score);
         if(this.var_104.event)
         {
            this.refreshTextWithCaption("startedat",_loc1_,"" + this.var_104.eventCreationTime);
         }
         this.var_2019.refreshThumbnail(_loc1_,this.var_104.thumbnail,false);
      ...
      private function onOwnerName(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(_loc3_ != null)
         {
            navigator.mainViewCtrl.startSearch(Tabs.const_251,Tabs.const_211,_loc3_.text);
            this.hideInstantly();
         }
      }
   }
