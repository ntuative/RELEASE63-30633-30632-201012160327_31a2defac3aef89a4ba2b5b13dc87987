package com.sulake.core.runtime.events
{
   ...
   public class EventDispatcher implements IEventDispatcher, IDisposable
   {
      ...
      private static const const_1196:uint = 1;
      
      private static const const_849:uint = 2;
       
      
      protected var _disposed:Boolean = false;
      
      private var var_214:flash.events.EventDispatcher;
      
      private var var_232:Dictionary;
      
      private var _result:uint;
      
      private var var_563:Error;
      
      public function EventDispatcher(param1:IEventDispatcher = null)
      {
         this.var_232 = new Dictionary();
         super();
         this.var_214 = new flash.events.EventDispatcher(!!param1 ? param1 : this);
      }
      ...
      public function addEventListener(param1:String, param2:Function, param3:Boolean = false, param4:int = 0, param5:Boolean = false) : void
      {
         ...
         if(!_loc6_)
         {
            ...
            this.var_214.addEventListener(param1,this.eventProcessor);
         }
         else
         {
            for each(_loc8_ in _loc6_)
            {
               if(_loc8_.callback == param2 && _loc8_.var_1860 == param3)
               {
                  return;
               }
      ...
      public function removeEventListener(param1:String, param2:Function, param3:Boolean = false) : void
      {
         ...
         if(!this._disposed)
         {
            ...
            if(_loc4_)
            {
               ...
               {
                  if(_loc6_.callback == param2 && _loc6_.var_1860 == param3)
                  {
      ...
      public function dispatchEvent(param1:Event) : Boolean
      {
         if(!this._disposed)
         {
            ...
            this.var_214.dispatchEvent(param1);
            if(this._result == const_849)
            {
               if(this.var_563 != null)
               {
                  Core.crash("Error catched when handling " + getQualifiedClassName(param1) + ": " + this.var_563.message,this.var_563.errorID,this.var_563);
               }
               else
               {
                  Core.crash("Error catched when handling " + getQualifiedClassName(param1) + ". No error data available!",0,this.var_563);
               }
            }
      ...
      private function eventProcessor(param1:Event) : void
      {
         ...
         this._result = !!event.isDefaultPrevented() ? uint(const_1196) : uint(const_549);
      }
