package com.sulake.core.communication.connection
{
   ...
   public class SocketConnection extends EventDispatcher implements IConnection, IDisposable
   {
      ...
      public static const const_1258:int = 10000;
      ...
      private var var_1149:IEncryption;
      
      private var var_746:IProtocol;
      
      private var _id:String;
      
      private var var_1148:IMessageClassManager;
      
      private var var_285:ICoreCommunicationManager;
      
      private var var_494:IConnectionStateListener;
      ...
      public function SocketConnection(param1:String, param2:ICoreCommunicationManager, param3:IConnectionStateListener)
      {
         ...
         this.var_1148 = new MessageClassManager();
         this.var_76 = new Socket();
         this.var_344 = new Timer(const_1258,1);
         this.var_344.addEventListener(TimerEvent.TIMER,this.onTimeOutTimer);
         this.var_76.addEventListener(Event.CONNECT,this.onConnect);
         this.var_76.addEventListener(Event.COMPLETE,this.onComplete);
         this.var_76.addEventListener(Event.CLOSE,this.onClose);
         this.var_76.addEventListener(ProgressEvent.SOCKET_DATA,this.onRead);
         this.var_76.addEventListener(SecurityErrorEvent.SECURITY_ERROR,this.onSecurityError);
         this.var_76.addEventListener(IOErrorEvent.IO_ERROR,this.onIOError);
         this.var_494 = param3;
      }
      ...
      public function dispose() : void
      {
         ...
         if(this.var_76)
         {
            this.var_76.removeEventListener(Event.CONNECT,this.onConnect);
            this.var_76.removeEventListener(Event.COMPLETE,this.onComplete);
            this.var_76.removeEventListener(Event.CLOSE,this.onClose);
            this.var_76.removeEventListener(ProgressEvent.SOCKET_DATA,this.onRead);
            this.var_76.removeEventListener(SecurityErrorEvent.SECURITY_ERROR,this.onSecurityError);
            this.var_76.removeEventListener(IOErrorEvent.IO_ERROR,this.onIOError);
            if(this.var_1841)
            {
               this.var_76.close();
            }
      ...
      public function init(param1:String, param2:uint = 0) : Boolean
      {
         if(this.var_494)
         {
            this.var_494.connectionInit(param1,param2);
         }
         Security.loadPolicyFile("xmlsocket://" + param1 + ":" + param2);
         this.var_344.start();
         this.var_747 = getTimer();
         this.var_76.connect(param1,param2);
         return true;
      }
      
      public function set timeout(param1:int) : void
      {
         this.var_344.delay = param1;
      }
      ...
      public function send(param1:IMessageComposer, param2:int = -1) : Boolean
      {
         ...
         var _loc4_:int = this.var_1148.getMessageComposerID(param1);
         if(_loc4_ < 0)
         {
            this.logConnection("Could not find registered message composer for " + param1);
            return false;
         }
         var _loc5_:ByteArray = this.var_746.encoder.encode(_loc4_,param1.getMessageArray(),param2);
         if(this.var_494)
         {
            this.var_494.messageSent(String(_loc4_),_loc5_.toString());
         }
         if(this.var_1149 != null)
         {
            _loc3_ = this.var_1149.encipher(_loc5_);
         }
         else
         {
            _loc3_ = _loc5_;
         ...
         if(this.var_76.connected)
         {
            this.var_76.writeBytes(_loc3_);
            this.var_76.flush();
            return true;
         }
      ...
      {
         this.var_1148.registerMessages(param1);
      }
      ...
      override public function toString() : String
      {
         ...
         _loc1_ += "Protocol Encoder: " + this.var_746.encoder + "\n";
         _loc1_ += "Protocol Decoder: " + this.var_746.decoder + "\n";
         return _loc1_ + ("Encryption: " + this.var_1149 + "\n");
      }
      ...
      public function processReceivedData() : void
      {
         ...
         var offset:uint = this.var_746.getMessages(this.var_495,receivedMessages);
         ...
         try
         {
            for each(message in receivedMessages)
            {
               ...
               if(this.var_494)
               {
                  this.var_494.messageReceived(String(id),data.toString());
               }
               this.logConnection(">>[SOCKET IN] timestamp:" + getTimer() + " Received message id: " + id + " data: " + data.toString());
               eventClasses = this.var_1148.getMessageEventClasses(id);
               events = new Array();
      ...
      private function onRead(param1:ProgressEvent) : void
      {
         ...
         while(this.var_76.bytesAvailable > 0)
         {
            this.var_495.writeByte(this.var_76.readUnsignedByte());
         }
      }
      ...
      private function onConnect(param1:Event) : void
      {
         ...
         this.var_344.stop();
         this.var_1841 = true;
         ErrorReportStorage.addDebugData("ConnectionTimer","Connected in " + (getTimer() - this.var_747));
      ...
      private function onClose(param1:Event) : void
      {
         ...
         this.var_1841 = false;
         ErrorReportStorage.addDebugData("ConnectionTimer","Closed in " + (getTimer() - this.var_747));
         dispatchEvent(param1);
      }
      ...
      private function onComplete(param1:Event) : void
      {
         ...
         ErrorReportStorage.addDebugData("ConnectionTimer","Completed in " + (getTimer() - this.var_747));
         dispatchEvent(param1);
      }
      ...
      private function onSecurityError(param1:SecurityErrorEvent) : void
      {
         ...
         ErrorReportStorage.addDebugData("ConnectionTimer","SecurityError in " + (getTimer() - this.var_747));
         dispatchEvent(param1);
      }
      ...
      private function onIOError(param1:IOErrorEvent) : void
      {
         ...
         ErrorReportStorage.addDebugData("ConnectionTimer","IOError in " + (getTimer() - this.var_747));
         switch(param1.type)
         {
      ...
      private function onTimeOutTimer(param1:TimerEvent) : void
      {
         ...
         ErrorReportStorage.addDebugData("ConnectionTimer","TimeOut in " + (getTimer() - this.var_747));
         var _loc2_:IOErrorEvent = new IOErrorEvent(IOErrorEvent.IO_ERROR);
         _loc2_.text = "Socket Timeout (" + this.var_344.delay + " ms). Possible Firewall.";
