package com.sulake.core.utils
{
   ...
   public class LibraryLoaderQueue extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1336:int = 4;
      ...
      {
         return this.var_355.length + this.var_255.length;
      }
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc1_ in this.var_355)
            {
               _loc1_.dispose();
            }
            this.var_255 = null;
      ...
      private function next() : void
      {
         ...
         if(!disposed)
         {
            while(this.var_255.length < const_1336 && this.var_355.length > 0)
            {
               _loc1_ = this.var_355.shift();
               this.var_255.push(_loc1_);
               _loc1_.resume();
            }
         }
      }
      ...
      private function removeLoader(param1:LibraryLoader) : void
      {
         ...
         try
         {
            index = this.var_355.indexOf(loader);
            if(index > -1)
            {
               this.var_355.splice(index,1);
            }
            index = this.var_255.indexOf(loader);
            if(index > -1)
            {
               this.var_255.splice(index,1);
            }
         }
         catch(e:Error)
         {
      ...
      private function isUrlInQueue(param1:String, param2:Boolean = true) : Boolean
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc3_ in this.var_355)
            {
               if(param2)
               {
      ...
      public function findLibraryLoaderByURL(param1:String, param2:Boolean = true) : LibraryLoader
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc3_ in this.var_255)
            {
               if(param2)
               {
                  if(_loc3_.url.indexOf(param1) == 0)
